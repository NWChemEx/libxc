...
#include "xc.cuh"
#include "xc_funcs_worker.cuh"

/* we include the references also */
#include "references.cuh"
...
__device__ double LambertW_cuda(double z);
__device__ double xc_dilogarithm_cuda(const double x);

/* we define this function here, so it can be properly inlined by all compilers */
__device__
static inline double
xc_cheb_eval_cuda(const double x, const double *cs, const int N)
{
  int i;
...
__device__ double xc_bessel_I0_scaled_cuda(const double x);
__device__ double xc_bessel_I0_cuda(const double x);
__device__ double xc_bessel_K0_scaled_cuda(const double x);
__device__ double xc_bessel_K0_cuda(const double x);
__device__ double xc_bessel_K1_scaled_cuda(const double x);
__device__ double xc_bessel_K1_cuda(const double x);

__device__ double xc_expint_e1_impl_cuda(double x, const int scale);
__device__ static inline double expint_e1_cuda(const double x)         { return  xc_expint_e1_impl( x, 0); }
__device__ static inline double expint_e1_scaled_cuda(const double x)  { return  xc_expint_e1_impl( x, 1); }
__device__ static inline double expint_Ei_cuda(const double x)         { return -xc_expint_e1_impl(-x, 0); }
#define Ei(x) expint_Ei(x)
__device__ static inline double expint_Ei_scaled_cuda(const double x)  { return -xc_expint_e1_impl(-x, 1); }

/* integration */
typedef void integr_fn(double *x, int n, void *ex);
__device__
double xc_integrate_cuda(integr_fn func, void *ex, double a, double b);
__device__
void xc_rdqagse_cuda(integr_fn f, void *ex, double *a, double *b, 
...
__device__
void xc_rho2dzeta_cuda(int nspin, const double *rho, double *d, double *zeta);

/* Functions to handle the internal counters */
__device__
void internal_counters_set_lda_cuda (int nspin, xc_dimensions *dim);
__device__
void internal_counters_lda_next_cuda
(const xc_dimensions *dim, int offset,
 const double **rho, double **zk, double **vrho, double **v2rho2, double **v3rho3
 );
__device__
void internal_counters_lda_prev_cuda
(const xc_dimensions *dim, int offset,
 const double **rho, double **zk, double **vrho, double **v2rho2, double **v3rho3
 );

__device__
void internal_counters_set_gga_cuda (int nspin, xc_dimensions *dim);
__device__
void internal_counters_gga_next_cuda
(const xc_dimensions *dim, int offset,
...
__device__
void internal_counters_gga_prev_cuda
(const xc_dimensions *dim, int offset,
 const double **rho, const double **sigma,
 double **zk,
 double **vrho, double **vsigma,
...
__device__
void internal_counters_set_mgga_cuda(int nspin, xc_dimensions *dim);

__device__
void internal_counters_mgga_next_cuda
(const xc_dimensions *dim, int offset,
 const double **rho, const double **sigma, const double **lapl, const double **tau,
 double **zk, MGGA_OUT_PARAMS_NO_EXC(double **));

__device__
void internal_counters_mgga_prev_cuda
(const xc_dimensions *dim, int offset,
 const double **rho, const double **sigma, const double **lapl, const double **tau,
 double **zk, MGGA_OUT_PARAMS_NO_EXC(double **));
...
__device__
void xc_mix_init_cuda(xc_func_type *p, int n_funcs, const int *funcs_id, const double *mix_coef);
__device__
...
__device__
const char *get_kind_cuda(const xc_func_type *func);
__device__
const char *get_family_cuda(const xc_func_type *func);
__device__
double get_ext_param_cuda(const func_params_type *params, const double *values, int index);
