...
DEVICE static double dvc_bi0_data[12] = {
...
DEVICE static double dvc_ai0_data[21] = {
...
DEVICE static double dvc_ai02_data[22] = {
...
DEVICE double dvc_xc_bessel_I0_scaled(const double x)
{
  ...
  if(y < 2.0*SQRT_DBL_EPSILON)
    ...
    r = exp(-y)*(2.75 + dvc_xc_cheb_eval(y*y/4.5-1.0, dvc_bi0_data, 12));
  else if(y <= 8.0)
    r = (.375 + dvc_xc_cheb_eval((48.0/y - 11.0)/5.0, dvc_ai0_data, 21))/sqrt(y);
  else
    r = (.375 + dvc_xc_cheb_eval(16.0/y - 1.0, dvc_ai02_data, 22))/sqrt(y);
...
DEVICE double dvc_xc_bessel_I0(const double x)
{
  ...
  if(y < 2.0*SQRT_DBL_EPSILON)
    ...
    r = 2.75 + dvc_xc_cheb_eval(y*y/4.5 - 1.0, dvc_bi0_data, 12);
  else if(y < LOG_DBL_MAX - 1.0)
    r = exp(y) * dvc_xc_bessel_I0_scaled(x);
  else {
    #ifndef __CUDACC__
...
DEVICE static double dvc_bi1_data[11] = {
...
DEVICE static double dvc_ai1_data[21] = {
...
DEVICE static double dvc_ai12_data[22] = {
...
DEVICE double dvc_xc_bessel_I1_scaled(const double x)
{
  ...
  if(y == 0.0)
    ...
    r = x*exp(-y)*(0.875 + dvc_xc_cheb_eval(y*y/4.5 - 1.0, dvc_bi1_data, 11));
  else{
    if(y <= 8.0)
      r = (0.375 + dvc_xc_cheb_eval((48.0/y - 11.0)/5.0, dvc_ai1_data, 21))/sqrt(y);
    else
      r = (0.375 + dvc_xc_cheb_eval(16.0/y - 1.0, dvc_ai12_data, 22))/sqrt(y);
...
DEVICE double dvc_xc_bessel_I1(const double x)
{
  ...
  if(y == 0.0)
    ...
    r = x*(0.875 + dvc_xc_cheb_eval(y*y/4.5 - 1.0, dvc_bi1_data, 11));
  else
    r = exp(x)*dvc_xc_bessel_I1_scaled(x);

  return r;
}

DEVICE static double dvc_bk0_data[11] = {
...
DEVICE static double dvc_ak0_data[17] = {
...
DEVICE static double dvc_ak02_data[14] = {
...
DEVICE double dvc_xc_bessel_K0_scaled(const double x)
{
  ...
  if(x <= 0.0) {
    ...
    r = exp(x)*(-log(0.5*x)*dvc_xc_bessel_I0(x) - 0.25 + dvc_xc_cheb_eval(0.5*x*x - 1.0, dvc_bk0_data, 11));
  else if(x <= 8.0)
    r = (1.25 + dvc_xc_cheb_eval((16.0/x - 5.0)/3.0, dvc_ak0_data, 17))/sqrt(x);
  else
    r = (1.25 + dvc_xc_cheb_eval(16.0/x - 1.0, dvc_ak02_data, 14))/sqrt(x);
...
DEVICE double dvc_xc_bessel_K0(const double x)
{
  ...
  if(x <= 0.0) {
    ...
    r = -log(0.5*x)*dvc_xc_bessel_I0(x) - 0.25 + dvc_xc_cheb_eval(0.5*x*x - 1.0, dvc_bk0_data, 11);
  else
    r = exp(-x)*dvc_xc_bessel_K0_scaled(x);

  return r;
}


DEVICE static double dvc_bk1_data[11] = {
...
DEVICE static double dvc_ak1_data[17] = {
...
DEVICE static double dvc_ak12_data[14] = {
...
DEVICE double dvc_xc_bessel_K1_scaled(const double x)
{
  ...
  if(x <= 0.0) {
    ...
    r =  exp(x)*(log(0.5*x)*dvc_xc_bessel_I1(x) +
		 (0.75 + dvc_xc_cheb_eval(.5*x*x - 1.0, dvc_bk1_data, 11))/x);
  else if(x <= 8.0)
    r = (1.25 + dvc_xc_cheb_eval((16.0/x - 5.0)/3.0, dvc_ak1_data, 17))/sqrt(x);
  else
    r = (1.25 + dvc_xc_cheb_eval(16.0/x - 1.0, dvc_ak12_data, 14))/sqrt(x);
...
DEVICE double dvc_xc_bessel_K1(const double x)
{
  ...
  if(x <= 0.0) {
    ...
    r = log(0.5*x)*dvc_xc_bessel_I1(x) + (0.75 + dvc_xc_cheb_eval(0.5*x*x - 1.0, dvc_bk1_data, 11))/x;
  else
    r = exp(-x)*dvc_xc_bessel_K1_scaled(x);
