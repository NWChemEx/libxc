...
{
    checkHipblas(__FILE__,__LINE__,hipblasCreate(&hipblas_handle));
    checkHipblas(__FILE__,__LINE__,hipblasSetPointerMode(hipblas_handle,HIPBLAS_POINTER_MODE_DEVICE));
}

void
xc_mix_func_end_hipblas()
{
    checkHipblas(__FILE__,__LINE__,hipblasDestroy(hipblas_handle));
}
            ...
            hipStream_t stream)
{
   ...
   *   output buffer.
   * - we need to use some hipBlas operation to add the terms as all the (double*)
  ...
  for(ii=0; ii<func->n_func_aux; ii++){
    ...
    */
    ...
    if(zk != NULL) {
      checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->zk,
                                                (xc_func_data_device+func_rank)->mix_coef+ii,
                                                zk_,1,zk,1));
    }

    if(vrho != NULL) {
      checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->vrho,
                                                (xc_func_data_device+func_rank)->mix_coef+ii,
                                                vrho_,1,vrho,1));
      if(is_gga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->vsigma,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      vsigma_,1,vsigma,1));
      }

      if(is_mgga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->vtau,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      vtau_,1,vtau,1));
        if(aux->info->flags & XC_FLAGS_NEEDS_LAPLACIAN) {
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->lapl,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        vlapl_,1,vlapl,1));
        }
      }
    ...
    if(v2rho2 != NULL){
      checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2rho2,
                                    (xc_func_data_device+func_rank)->mix_coef+ii,
                                    v2rho2_,1,v2rho2,1));

      if(is_gga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2rhosigma,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v2rhosigma_,1,v2rhosigma,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2sigma2,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v2sigma2_,1,v2sigma2,1));
      }

      if(is_mgga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2rhotau,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v2rhotau_,1,v2rhotau,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2sigmatau,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v2sigmatau_,1,v2sigmatau,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2tau2,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v2tau2_,1,v2tau2,1));
        if(aux->info->flags & XC_FLAGS_NEEDS_LAPLACIAN) {
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2rholapl,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v2rholapl_,1,v2rholapl,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2lapl2,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v2lapl2_,1,v2lapl2,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2sigmalapl,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v2sigmalapl_,1,v2sigmalapl,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v2lapltau,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
    ...
    if(v3rho3 != NULL){
      checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rho3,
                                    (xc_func_data_device+func_rank)->mix_coef+ii,
                                    v3rho3_,1,v3rho3,1));
      
      if(is_gga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rho2sigma,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3rho2sigma_,1,v3rho2sigma,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rhosigma2,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3rhosigma2_,1,v3rhosigma2,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigma3,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3sigma3_,1,v3sigma3,1));
      }
      if(is_mgga(aux->info->family)) {
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rho2tau,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3rho2tau_,1,v3rho2tau,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rhotau2,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3rhotau2_,1,v3rhotau2,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigma2tau,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3sigma2tau_,1,v3sigma2tau,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigmatau2,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3sigmatau2_,1,v3sigmatau2,1));
        checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3tau3,
                                      (xc_func_data_device+func_rank)->mix_coef+ii,
                                      v3tau3_,1,v3tau3,1));

        if(aux->info->flags & XC_FLAGS_NEEDS_LAPLACIAN) {
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rho2lapl,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3rho2lapl_,1,v3rho2lapl,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rholapl2,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3rholapl2_,1,v3rholapl2,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3rholapltau,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3rholapltau_,1,v3rholapltau,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigma2lapl,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3sigma2lapl_,1,v3sigma2lapl,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigmalapl2,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3sigmalapl2_,1,v3sigmalapl2,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3sigmalapltau,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3sigmalapltau_,1,v3sigmalapltau,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3lapl3,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3lapl3_,1,v3lapl3,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3lapl2tau,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
                                        v3lapl2tau_,1,v3lapl2tau,1));
          checkHipblas(__FILE__,__LINE__,hipblasDaxpy(hipblas_handle,np*dim->v3lapltau2,
                                        (xc_func_data_device+func_rank)->mix_coef+ii,
  ...
  checkHipblas(__FILE__,__LINE__,hipblasSetStream(hipblas_handle,NULL));
