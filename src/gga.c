	     ...
	     double *v3rho3, double *v3rho2sigma, double *v3rhosigma2, double *v3sigma3)
{
  ...
  if(zk != NULL && !(func->info->flags & XC_FLAGS_HAVE_EXC)){
    printf("Functional '%s' does not provide an implementation of Exc\n",
	    func->info->name);
    exit(1);
  }

  if(vrho != NULL && !(func->info->flags & XC_FLAGS_HAVE_VXC)){
    printf("Functional '%s' does not provide an implementation of vxc\n",
	    func->info->name);
    exit(1);
  }

  if(v2rho2 != NULL && !(func->info->flags & XC_FLAGS_HAVE_FXC)){
    printf("Functional '%s' does not provide an implementation of fxc\n",
	    func->info->name);
    exit(1);
  }

  if(v3rho3 != NULL && !(func->info->flags & XC_FLAGS_HAVE_KXC)){
    printf("Functional '%s' does not provide an implementation of kxc\n",
	    func->info->name);
    exit(1);
  }
